...
class Feitico {
  ...
  factory Feitico.fromJson(Map<String, dynamic> json) => Feitico(
        nome: json['nome'],
...
class Mago {
  ...
  factory Mago.fromJson(Map<String, dynamic> json) => Mago(
        nome: json['nome'],
        idade: json['idade'],
        feiticos: (json['feiticos'] as List)
            .map((f) => Feitico.fromJson(f))
            .toList(),
      );
}

void main() {
  runApp(const MagosApp());
}

class MagosApp extends StatelessWidget {
  const MagosApp({super.key});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MagosListScreen(),
    );
  }
}

class MagosListScreen extends StatelessWidget {
  const MagosListScreen({super.key});

  List<Mago> _parseMagos() {
    final decoded = jsonDecode(jsonString);
    return (decoded['magos'] as List)
        .map((m) => Mago.fromJson(m))
        .toList();
  }

  @override
  Widget build(BuildContext context) {
    final magos = _parseMagos();

    return Scaffold(
      appBar: AppBar(title: const Text('Magos')),
          ...
          return ListTile(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => DetalhesMagoScreen(mago: mago),
                ),
              );
            },
          );
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              trailing: Text(feitico.elemento),
            ),
          );
        },
      ),
